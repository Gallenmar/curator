  ...
  const calculateConsumption = (readings: typeof mockReadings) => {
    ...
    for (let i = 1; i < sorted.length; i++) {
      consumption.push({
        ...
        hotWater: parseFloat((sorted[i].hotWater - sorted[i - 1].hotWater).toFixed(2)),
        coldWater: parseFloat((sorted[i].coldWater - sorted[i - 1].coldWater).toFixed(2)),
      });
    }
  ...
  return (
    <Layout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Water Meter Readings</h1>
